...
async function sendWhatsAppWelcome() {
  const result = await notificationService.sendWhatsApp({
    ...
    message: 'Welcome! We are excited to have you on board. Reply to this message if you need any help.',
  });
...
async function sendNotificationToPreferredChannel(userId: string, preferredChannel: 'email' | 'sms' | 'whatsapp') {
