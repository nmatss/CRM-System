...
const firstNames = [
  ...
  'Sofia', 'Thiago', 'Valentina', 'William', 'Beatriz', 'Caio', 'Diana', 'Felipe',
  'Giovanna', 'Henrique', 'Isabella', 'João', 'Larissa', 'Mateus', 'Natalia', 'Otavio',
  'Paula', 'Rafael', 'Sandra', 'Tiago', 'Vanessa', 'Yuri', 'Amanda', 'Bernardo',
  'Camila', 'Diego', 'Elisa', 'Fabio', 'Gabriela', 'Hugo', 'Isabela', 'Jose',
  'Karen', 'Leonardo', 'Melissa', 'Nathan', 'Patricia', 'Ricardo', 'Silvia', 'Victor'
];

const lastNames = [
  'Silva', 'Santos', 'Oliveira', 'Souza', 'Rodrigues', 'Ferreira', 'Alves', 'Pereira',
  'Lima', 'Gomes', 'Costa', 'Ribeiro', 'Martins', 'Carvalho', 'Rocha', 'Almeida',
  'Nascimento', 'Araujo', 'Melo', 'Barbosa', 'Cardoso', 'Correia', 'Dias', 'Fernandes',
  'Garcia', 'Mendes', 'Monteiro', 'Moreira', 'Nunes', 'Pinto', 'Ramos', 'Reis',
  'Ribeiro', 'Santana', 'Teixeira', 'Vieira', 'Freitas', 'Castro', 'Pires', 'Azevedo'
];

const productNames = {
  Roupas: [
    'Camiseta Básica', 'Calça Jeans', 'Vestido Floral', 'Blusa Social', 'Shorts Jeans',
    'Jaqueta de Couro', 'Moletom com Capuz', 'Saia Midi', 'Camisa Polo', 'Blazer Executivo',
    'Legging Fitness', 'Regata Esportiva', 'Calça Alfaiataria', 'Top Cropped', 'Bermuda Tactel'
  ],
  Eletrônicos: [
    'Fone de Ouvido Bluetooth', 'Smartwatch', 'Carregador Portátil', 'Mouse Gamer',
    'Teclado Mecânico', 'Webcam HD', 'Suporte para Notebook', 'Caixa de Som Bluetooth',
    'Cabo USB-C', 'Adaptador HDMI', 'Pendrive 64GB', 'HD Externo 1TB',
    'Mousepad RGB', 'Ring Light', 'Microfone Condensador'
  ],
  Acessórios: [
    'Relógio Digital', 'Óculos de Sol', 'Bolsa Transversal', 'Carteira de Couro',
    'Cinto Masculino', 'Mochila Executiva', 'Pulseira de Prata', 'Brinco Argola',
    'Colar Folheado', 'Boné Trucker', 'Lenço de Seda', 'Chaveiro Personalizado',
    'Porta-Cartões', 'Necessaire', 'Guarda-Chuva Automático'
  ],
  Casa: [
    'Jogo de Cama Casal', 'Toalha de Banho', 'Tapete Decorativo', 'Almofada Decorativa',
    'Cortina Blackout', 'Organizador Multiuso', 'Conjunto de Panelas', 'Jogo de Copos',
    'Luminária de Mesa', 'Espelho Decorativo', 'Quadro Decorativo', 'Vaso Cerâmica',
    'Difusor de Aromas', 'Porta Retratos', 'Cesto de Roupa'
  ],
  Beleza: [
    'Kit Maquiagem', 'Perfume Importado', 'Creme Hidratante', 'Shampoo Premium',
    'Condicionador', 'Máscara Facial', 'Sérum Vitamina C', 'Batom Matte',
    'Base Líquida', 'Pó Compacto', 'Paleta de Sombras', 'Rímel', 'Delineador',
    'Esmalte Gel', 'Removedor de Maquiagem'
  ]
};

const segments = ['VIP', 'Frequente', 'Ocasional', 'Novo', 'Inativo'];
const segmentDistribution = [0.10, 0.25, 0.40, 0.15, 0.10]; // Percentages

const orderStatuses = ['Pendente', 'Processando', 'Enviado', 'Entregue', 'Cancelado'];
const statusDistribution = [0.05, 0.10, 0.15, 0.60, 0.10]; // Percentages
...
export async function seedCustomers(tenantId: number, count: number = 100): Promise<number[]> {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const createdAt = randomDate(twelveMonthsAgo, now);
...
export async function seedCashbackTransactions(
  ...
  const transactions: InsertCashbackTransaction[] = [];
  const now = new Date();
  ...
  for (const customerId of customersWithCashback) {
    let balance = 0;
    const transactionCount = randomInt(1, 5);
    ...
    for (let i = 0; i < transactionCount; i++) {
      ...
      if (type === 'credit') {
        balance += amount;
        const expiresAt = addMonths(now, randomInt(1, 3));

        transactions.push({
          tenantId,
          customerId,
          ruleId: randomElement(ruleIds),
          orderId: Math.random() > 0.5 ? randomElement(orderIds) : null,
          type,
          amount,
          balance,
        ...
        transactions.push({
          ...
          customerId,
          ruleId: null,
          orderId: randomElement(orderIds),
          type,
          amount,
          balance,
  ...
  for (const transaction of transactions) {
    await storage.createCashbackTransaction(transaction);
  }

  console.log(`✅ Created ${transactions.length} cashback transactions`);
}
