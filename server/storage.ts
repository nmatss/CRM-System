...
// The contract lives in ./storage/contracts; re-exported so existing importers
// of `server/storage` keep working unchanged.
export * from "./storage/contracts";

/**
 * SQLite implementation of the storage contract.
 *
 * The contract, domain errors and audit policy live in `./storage/contracts`.
 * The methods below are deliberately still one class: they share private
 * helpers and transaction scopes through `this`, and the highest-risk code in
 * the system (the cashback ledger and the order/stock transaction) lives here.
 * Splitting it into per-domain repositories is a real architectural change,
 * not a file move, and it is tracked as such in the completion plan rather than
 * being forced through mechanically.
 */
