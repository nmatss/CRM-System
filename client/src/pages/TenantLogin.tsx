...
export default function TenantLogin() {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      if (tenant && data.user) {
        try {
          await apiRequest("POST", `/auth/switch-tenant/${tenant.id}`);
        } catch (e) {
        }
      }
  ...
  if (tenantError || !tenant) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-50 to-pink-50 px-4 py-8">
        <Card className="w-full max-w-sm sm:max-w-md text-center" data-testid="tenant-not-found-card">
          <CardHeader className="px-4 sm:px-6 pt-6 sm:pt-8">
            ...
            <CardTitle className="text-xl sm:text-2xl font-bold text-gray-600">Loja não encontrada</CardTitle>
    ...
    >
      <Card className="w-full max-w-sm sm:max-w-md" data-testid="tenant-login-card">
        <CardHeader className="text-center px-4 sm:px-6 pt-6 sm:pt-8">
          ...
          <CardTitle className="text-xl sm:text-2xl font-bold" data-testid="tenant-name">{tenant.name}</CardTitle>
          <CardDescription className="text-sm sm:text-base">
        ...
        <CardContent className="px-4 sm:px-6 pb-6">
          <form onSubmit={handleLogin} className="space-y-4 sm:space-y-5">
            <div className="space-y-2">
              <Label htmlFor="login-username" className="text-sm sm:text-base">CPF ou Email</Label>
      ...
      <Dialog open={showPasswordModal} onOpenChange={() => {}}>
        <DialogContent className="w-[calc(100%-2rem)] max-w-sm sm:max-w-md" data-testid="password-change-modal">
          <DialogHeader className="px-1">
            <div className="flex justify-center mb-3 sm:mb-4">
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl flex items-center justify-center" style={gradientStyle}>
